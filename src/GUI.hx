...
@:publicFields
class GUI {
    static inline var SLIDER_HEIGHT_SCALE = 2.0;
	...
	static function image_button(x: Float, y: Float, image: String, button_function: Void->Void) {
		...
		var imageheight = Gfx.imageheight(image);
		var button_width = image_width * 1.1;
		var button_height = imageheight * 1.1;
	...
	static function auto_slider(text: String, set_function: Float->Void, current: Float, min: Float, max: Float, 
		...
		var text_width = Text.width(text);
		var text_height = Text.height();
		var height = text_height * SLIDER_HEIGHT_SCALE;
		y += (height + 2) * (skips);
	...
	static function slider(slider_x: Float, slider_y: Float, text: String, set_function: Float->Void, current: Float,
		...
		var value_string = '${Math.round(current)}';
		Text.display(slider_x + area_width / 2 - Text.width(value_string) / 2, y + height / 4, value_string, Col.WHITE);
		Text.display(slider_x + area_width + handle_width, y + height / 4, text);
	}
